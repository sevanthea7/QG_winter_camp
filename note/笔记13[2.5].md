...
- 作用是设置随机数生成器的种子（seed），以确保代码的可重现性。
- 具体来说，它指定了随机数生成器的初始状态，使得每次运行代码时生成的随机数序列都是相同的。通过设置了相同的 `random_state` 参数，每次调用 `preprocess_data` 函数时都会使用相同的随机数种子，从而确保每次划分训练集和测试集时都使用相同的随机排列顺序，从而得到相同的结果。
...
- `np.ones((X.shape[0], 1))` 创建了一个形状为 `(X.shape[0], 1)` 的全为1的矩阵，表示截距项
  - 对于NumPy数组，`.shape` 属性返回一个元组，其中包含数组的维度信息。对于二维数组（矩阵）而言，`.shape[0]` 表示矩阵的行数，也就是样本的数量。因为每一行代表一个样本，所以行数即为样本的数量。
  - 举例来说，如果 `X` 是一个形状为 `(m, n)` 的二维数组，其中 `m` 表示行数（样本数量），`n` 表示列数（特征数量），那么 `X.shape[0]` 就等于 `m`，即样本的数量。
